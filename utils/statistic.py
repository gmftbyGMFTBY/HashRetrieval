...
if __name__ == "__main__":
    ...
    print(f'[Train] {dataset} size: {len(dataset_)}')
    max_l, min_l, avg_l = session_size(dataset_)
    print(f'[Train] {dataset} turn size (max|min|avg): {max_l}|{min_l}|{avg_l}')
    max_l, min_l, avg_l = utterance_size(dataset_)
    print(f'[Train] {dataset} words in utterance (max|min|avg): {max_l}|{min_l}|{avg_l}')
    # coherence_ratio = coherent_check(dataset_)
    # print(f'[Train] {dataset} coherencey ratio: {coherence_ratio}')
    
    dataset_ = read_corpus(f'data/{dataset}/test.txt')
    print(f'[Test] {dataset} size: {len(dataset_)}')
    max_l, min_l, avg_l = session_size(dataset_)
    print(f'[Test] {dataset} turn size (max|min|avg): {max_l}|{min_l}|{avg_l}')
    max_l, min_l, avg_l = utterance_size(dataset_)
    print(f'[Test] {dataset} words in utterance (max|min|avg): {max_l}|{min_l}|{avg_l}')
    # coherence_ratio = coherent_check(dataset_)
    # print(f'[Test] {dataset} coherencey ratio: {coherence_ratio}')